  ...
  gazebo_msgs__msg__EntityState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    gazebo_msgs__msg__EntityState * data =
      (gazebo_msgs__msg__EntityState *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gazebo_msgs__msg__EntityState__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gazebo_msgs__msg__EntityState__fini(&data[i]);
        }
        free(data);
        return false;
      }
