  ...
  gazebo_msgs__msg__PerformanceMetrics__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    gazebo_msgs__msg__PerformanceMetrics * data =
      (gazebo_msgs__msg__PerformanceMetrics *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gazebo_msgs__msg__PerformanceMetrics__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gazebo_msgs__msg__PerformanceMetrics__fini(&data[i]);
        }
        free(data);
        return false;
      }
