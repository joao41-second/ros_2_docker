  ...
  gazebo_msgs__msg__ContactState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    gazebo_msgs__msg__ContactState * data =
      (gazebo_msgs__msg__ContactState *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gazebo_msgs__msg__ContactState__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gazebo_msgs__msg__ContactState__fini(&data[i]);
        }
        free(data);
        return false;
      }
